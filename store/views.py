...
# from cart.cart import Cart TODO
...
class Index(ListView):
    model = Product
    template_name = '../templates/store/index.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        # cart = Cart(request=self.request) TODO
        # context['my_price'] = cart.get_total_price() TODO
...
def product_by_category(request, category_id, slug):
    ...
    # endregion
    categories = Category.objects.filter(parent__isnull=True).annotate(product_count=Count('products'))
    paginator = Paginator(products, 6)
    page_number = request.GET.get('page')
    page_obj = paginator.get_page(page_number)
    top_sell = Product.objects.all().order_by('-sell')[:3]
    ...
    return render(request, '../templates/store/by_category.html', context)
...
def product_detail(request, product_id, product_slug):
    ...
    return render(request, '../templates/store/detail.html', context)
...
def search(request):
    ...
    return render(request, '../templates/store/search.html', {
        'products': page_obj,
        'query': query,
        'page_obj': page_obj,
    })
...
def list_favorite(request):
    ...
    return render(request, '../templates/store/list_favorite.html', {'products': products})
...
def remove_favorite(request, id):
    if request.method == 'POST':
        ...
        saved = False
        favorite_count = FavoriteProduct.objects.filter(user=request.user).count()
        return JsonResponse({'success': True, 'message': 'محصول با موفقیت حذف شد', 'favorite_count': favorite_count, 'saved': saved})
    return JsonResponse({'success': False, 'error': 'درخواست نامعتبر یا کاربر احراز هویت نشده است'})


def about_us(request):
    return render(request, '../templates/store/about_us.html')
...
def contact_us(request):
    if request.method == 'POST':
        ...
        form = ContactUsForm()
    return render(request, '../templates/store/about_us.html', {'form': form})
...
def emails(request):
    if request.method == 'POST':
        ...
        form = EmailForm()
    return render(request, '../templates/partials/footer.html', {'form': form})
